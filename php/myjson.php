...
class JsonPack
{
	...
	static function WriteIntSingle(&$binstr_writebuf,&$seek,$number)
	{
		$bytelen = 1;
		$c = (int)$number;
		while ($c >= 0x100)
	...
	{
		JsonPack::WriteIntSingle($binstr_writebuf,$seek,count($dict));
		for($i=0;$i<count($dict);$i++)
		{
			$str=$dict[$i];
			JsonPack::WriteIntSingle($binstr_writebuf,$seek,strlen($str));
			$binstr_writebuf.=$str;
			$seek+=strlen($str);
		}
	}
	...
	static function MakeNumberTag($isFloat, $isBool, $isNull,$datalength)
	{
		$tag = 128 | 0;//numbertag
		if ($isFloat)
			$tag |= 32;
    ...
    {
		$tag = JsonPack::MakeNumberTag(true, false, false, 4);
		$binstr_writebuf.=pack("C",$tag);
		$seek++;
	...
	static function WriteIntData(&$binstr_writebuf,&$seek, $number)
    {
        ...
        $tag=JsonPack::MakeNumberTag(false, false, false, $bytelen);
		$binstr_writebuf.=pack("C",$tag);
		$seek++;
		JsonPack::WriteIntPiece($binstr_writebuf,$seek,$bytelen,$number);
    }
	...
	static function PackJsonNumber(&$binstr_writebuf,&$seek,$number)
	{
		if ($number===null)
		{
			$binstr_writebuf.=pack("C",JsonPack::MakeNumberTag(false, false, true, 0));
			$seek++;
		}
		elseif (is_bool($number)==true)
		{
			$binstr_writebuf.=pack("C",JsonPack::MakeNumberTag(false, true, $number, 0));
			$seek++;
		}
